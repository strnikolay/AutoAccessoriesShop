import "./screen3.css"

function MainThirdScreen () {

return(
  
  <div class="screen3">
    <div class="item--an3">
    <div class="fon-HAf">
      <img class="porsche-normal-1-qCB" src="./assets/porsche-normal-1.png"/>
      <div class="rectangle-51-MgK">
      </div>
      <div class="content-4qd">
        <img class="rectangle-53-Psu" src="./assets/rectangle-53-rPq.png"/>
        <p class="pro-pro-20--jgs">
        Мы рады приветствовать вас на нашем сайте.
        <br/>
        
        <br/>
        
        <br/>
        
        <br/>
        Компания «PROавто» - это надежные, качественные аккумуляторы в широком ассортименте, разнообразные в по форме и характеристикам, а также качественные масла и автохимия именитых брендов.
        <br/>
        
        <br/>
        
        <br/>
        
        <br/>
        Сеть магазинов «PROавто» более 20-и лет занимается продажей автомобильных аккумуляторов, масел, расходных материалов, химией и аксессуаров. Мы зарекомендовали себя как надежная компания, специалисты которой всегда готовы проконсультировать по любому вопросу: от подбора автощёток, до рекомендаций по подбору аккумулятора для Вашего автомобиля. Мы дорожим своей репутацией и даём гарантию на весь ассортимент продукции, представленной на нашем сайте!
        <br/>
        
        <br/>
        
        <br/>
        
        <br/>
        Убедитесь в этом сами!
        </p>
      </div>
      <div class="item--775">
        <div class="ico-2-DA7">
          <img class="vector-45-6zb" src="./assets/vector-45.png"/>
          <div class="group-29-S2s">
            <img class="vector-46-ZNP" src="REPLACE_IMAGE:284:30"/>
            <img class="vector-47-hjV" src="REPLACE_IMAGE:284:31"/>
            <img class="vector-52-qao" src="REPLACE_IMAGE:284:36"/>
            <img class="vector-48-mzF" src="REPLACE_IMAGE:284:32"/>
            <img class="vector-50-iuV" src="REPLACE_IMAGE:284:34"/>
            <img class="vector-49-GRD" src="REPLACE_IMAGE:284:33"/>
            <img class="vector-51-og3" src="REPLACE_IMAGE:284:35"/>
          </div>
        </div>
        <p class="item--WqM">Реквизиты</p>
      </div>
      <div class="item--p5M">
        <img class="ico-1-7aF" src="./assets/ico-1.png"/>
        <p class="item--Raw">Задать вопрос</p>
      </div>
      <div class="item--KgK">
        <div class="item--dBD">о компании</div>
        <div class="rectangle-25-iyM">
        </div>
      </div>
    </div>
  </div>
    
  </div>

)}

export default MainThirdScreen;   
